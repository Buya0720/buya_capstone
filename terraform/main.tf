...
#     AIRFLOW__DATABASE__SQL_ALCHEMY_CONN=postgresql+psycopg2://airflow:airflow@3.107.84.178:5432/airflow_db
#     ENV
...
module "code_bucket" {
  ...
  project     = var.project
  environment = var.environment
  bucket_name = "code-${var.environment}-buya"
}
...
module "data_bucket" {
  ...
  project     = var.project
  environment = var.environment
  bucket_name = "${var.project}-${var.environment}-buya"
}
